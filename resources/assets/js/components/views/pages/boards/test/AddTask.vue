    ...
    <transition name="fade">            
        <div class="overlay">
            <div class="close-mod-btn">
                <button @click.prevent="$router.go(-1)" class="btn btn-simple btn-just-icon btn-default" title="Close"><i class="fa fa-close"></i></button>
            </div>
    ...
    methods: {
        ...
        addTask() {
            ...
            this.$store.dispatch('addSprintTask', task)
                .then((response) => {
                    this.$store.commit('addCusTask', response);
